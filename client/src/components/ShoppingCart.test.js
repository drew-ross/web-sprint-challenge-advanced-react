...
test("ShoppingCart renders and displays plants through props", () => {

    ///Arrange
    render(<ShoppingCart cart={plantsData} />);

    ///Act
    //get all plants on screen by name in mock data
    const plants = plantsData.map(plant => screen.getByText(plant.name));
    
    ///Assert
    //assert that each plant in mock data is also in the document
    plants.forEach(plant => expect(plant).toBeInTheDocument());
